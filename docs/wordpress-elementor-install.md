...
This project includes both a Python CLI (`tools/resume_builder.py`) and a web API (`webapp/app.py`). WordPress still needs the API layer because Elementor cannot directly execute Python code.
...
Install dependencies and run the included web app:
...
The API is now provided directly by `webapp/app.py` with:
